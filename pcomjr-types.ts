...
export type CIASTerminalId = Extract<
  TerminalId,
  'sonic-genesis' | 'da-cypher' | 'talon-vision' | 'talon-motion' | 'talon-fly' | 'talon-learn'
>;

/** OIAS systems operate on the organizational meta-layer. */
export type OIASSystemId = Extract<
  TerminalId,
  'artifact-memory' | 'decision-intelligence' | 'pow-ledger' | 'talon-agent'
>;
...
export type ArtifactType =
  ...
  // TalonLearn
  | 'learning-module'
  // Artifact Memory (OIAS)
  | 'knowledge-node'
  | 'session-brief'
  | 'constellation-cluster'
  | 'relationship-edge'
  | 'transcript-ingestion'
  // Decision Intelligence OS (OIAS)
  | 'decision-template'
  | 'decision-review'
  | 'governance-policy'
  | 'approval-chain'
  ...
  // Catch-all for future terminals
  | 'generic';
...
// ─────────────────────────────────────────────
// Core Entity: Artifact (base class)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// CIAS Domain Artifact Subclasses
// ─────────────────────────────────────────────
...
export interface Beat extends Artifact {
  ...
  content: {
    ...
    stems?: string[];       // URLs/paths to separated stems
    generationModel: string; // e.g., 'lyria-3-pro'
  };
}
...
export interface LoopSection {
  ...
  key: string;
  durationSeconds: number;
  beatId?: string; // References the Beat artifact for this section
}
...
export interface AnimationClip extends Artifact {
  ...
  content: {
    sourceArtifactIds: string[]; // Comic panels/pages this was derived from
    durationSeconds: number;
    resolution: string;
    format: string;
...
export interface MarketplaceListing extends Artifact {
  ...
  content: {
    ...
    currency: string;
    creatorId: string;
    sourceArtifactIds: string[]; // What's being sold
...
export interface MemoryBusEvent<T extends Artifact = Artifact> {
  ...
  /** What happened. */
  eventType: 'artifact.created' | 'artifact.updated' | 'artifact.sealed' |
             'decision.captured' | 'verification.completed' | 'pattern.detected' |
             'workflow.proposed' | 'workflow.adopted';
...
export function isCIASArtifact(a: Artifact): boolean {
  const ciasTerminals: TerminalId[] = [
