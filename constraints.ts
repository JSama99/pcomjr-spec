...
function isValidHash(val: unknown): val is string {
...
const VALID_TERMINALS: TerminalId[] = [
...
const TERMINAL_ALLOWED_TYPES: Record<TerminalId, ArtifactType[]> = {
...
export function validateArtifact(a: Artifact): Violation[] {
  const v: Violation[] = [];
...
export function validateDecision(d: Decision): Violation[] {
...
export function validateContext(ctx: Context, prefix = ''): Violation[] {
  const v: Violation[] = [];
...
export function validateEvidence(ev: Evidence, prefix = ''): Violation[] {
  const v: Violation[] = [];
  const p = prefix ? `${prefix}.` : '';
...
export function validateVerification(ver: Verification): Violation[] {
  const v: Violation[] = [];
  ...
  const validStatuses: string[] = ['pending', 'verified', 'failed', 'tampered'];
...
export function validateLedgerEntry(entry: LedgerEntry, expectedCaptureAgent?: string): Violation[] {
  const v: Violation[] = [];
...
export function validatePattern(p: Pattern): Violation[] {
  const v: Violation[] = [];
...
export function validateMemoryBusEvent(event: MemoryBusEvent): Violation[] {
  const v: Violation[] = [];
 ...
 */
export function validate(a: Artifact): ValidationResult {
  let violations: Violation[];

  switch (a.type) {
    case 'decision':
      violations = validateDecision(a as Decision);
      break;
    default:
 ...
 */
export function validateOrThrow(a: Artifact): void {
